...
CREATE TABLE sexo ( -- comando para criar tabela
    codsexo INTEGER IDENTITY  PRIMARY KEY, -- comando para criar chave primaria 
    nomesexo VARCHAR(80) NOT NULL UNIQUE -- comando para criar um campo de texto, e não pode ser repetido;
);
...
CREATE TABLE cidade (
    ...
    coduf_fk INTEGER NOT NULL REFERENCES uf(coduf)  
);
...
-- CLIENTE = {@CODCLIENTE, NOMECLIENTE, FOTO, DATANASC, #CODSEXO_fk, #CODRUA_fk, #CODBAIRRO_fk, #CODCEP_fk, #CODCIDADE_fk, SALARIO, #CODTRABALHO_fk, NUMEROCASA}
CREATE TABLE cliente (
    codcliente INTEGER IDENTITY PRIMARY KEY,
    nomecliente VARCHAR(80) NOT NULL,
    foto VARBINARY(MAX),
    datanasc DATE NOT NULL,
    codsexo_fk INTEGER NOT NULL REFERENCES sexo(codsexo),
    codrua_fk INTEGER NOT NULL REFERENCES rua(codrua),
    codbairro_fk INTEGER NOT NULL REFERENCES bairro(codbairro),
    codcep_fk INTEGER NOT NULL REFERENCES cep(codcep),
    codcidade_fk INTEGER NOT NULL REFERENCES cidade(codcidade),
    salario numeric(10,2) NOT NULL,
    codtrabalho_fk INTEGER NOT NULL REFERENCES trabalho(codtrabalho),
...
-- TELEFONE = {@CODTELEFONE, NUMERO, #CODOPERADORA_fk}
CREATE TABLE telefone (
    codtelefone INTEGER IDENTITY PRIMARY KEY,
    numero VARCHAR(14) NOT NULL,
    codoperadora_fk INTEGER NOT NULL REFERENCES operadora(codoperadora)
);


-- ITENSTELCLIENTE = {@(#CODTELEFONE_fk, #CODCLIENTE_fk)}
...
-- ITENSTELEFONETRABALHO = {@(#CODTRABALHO_fk, #CODTELEFONE_fk)}
CREATE TABLE itenstelefonetrabalho (
    codtrabalho_fk INTEGER REFERENCES trabalho(codtrabalho),
    codtelefone_fk INTEGER REFERENCES telefone(codtelefone),
    PRIMARY KEY (codtrabalho_fk, codtelefone_fk)
);
...
CREATE TABLE loja (
    ...
    nomeloja VARCHAR(80) NOT NULL UNIQUE,
...
-- FUNCIONARIOS = {@CODFUNCIONARIO, NOMEFUNCIONARIO, NUMEROCASA, #CODRUA_fk, #CODBAIRRO_fk, #CODCEP_fk, #CODCIDADE_fk, #CODFUNCAO_fk, SALARIO, #CODLOJA_fk}
CREATE TABLE funcionarios (
    ...
    codcep_fk INTEGER NOT NULL REFERENCES cep(codcep),
    codcidade_fk INTEGER NOT NULL REFERENCES cidade(codcidade),
    codfuncao_fk INTEGER NOT NULL REFERENCES funcao(codfuncao),
    salario numeric(8,2) NOT NULL,
    codloja_fk INTEGER NOT NULL REFERENCES loja(codloja)
);
...
-- CONTROLELOGSISTEMA = {@CODCONTROLE, #CODLOGIN_fk, DATA, HORA}
CREATE TABLE controlelogsistema (
    codcontrole INTEGER IDENTITY PRIMARY KEY,
    codlogin_fk INTEGER NOT NULL REFERENCES logar(codlogin),
...
-- ITENSACESSOLOGIN = {@(#CODACESSO_fk, #CODLOGIN_fk)}
CREATE TABLE itensacessologin (
    codacesso_fk INTEGER NOT NULL REFERENCES acesso(codacesso),
    codlogin_fk INTEGER NOT NULL REFERENCES logar(codlogin),
    PRIMARY KEY (codacesso_fk, codlogin_fk)
);


-- ITENSTELEFONELOJA={@(#CODLOJA_fk, #CODTELEFONE_fk)}
CREATE TABLE itenstelefoneloja (
    codloja_fk INTEGER NOT NULL REFERENCES loja(codloja),
    codtelefone_fk INTEGER NOT NULL REFERENCES telefone(codtelefone),
    PRIMARY KEY (codloja_fk, codtelefone_fk)
);
...
CREATE TABLE produto (
    ...
    codmarca_fk INTEGER NOT NULL REFERENCES marca(codmarca),
    codtipo_fk INTEGER NOT NULL REFERENCES tipo(codtipo)
);


-- FORNECEDOR={@CODFORNECEDOR, NOMEFORNECEDOR, NUMEROCASA, #CODRUA_fk, #CODBAIRRO_fk, #CODCEP_fk, #CODCIDADE_fk}
...
-- ITENSTELEFONEFORNECEDOR={@(#CODTELEFONE_fk, #CODFORNECEDOR_fk)}
CREATE TABLE itenstelefonefornecedor (
    codtelefone_fk INTEGER NOT NULL REFERENCES telefone(codtelefone),
    codfornecedor_fk INTEGER NOT NULL REFERENCES fornecedor(codfornecedor),
    PRIMARY KEY (codtelefone_fk, codfornecedor_fk)
);


-- COMPRAPRODUTO = {@CODCOMPRA, DATACOMPRA, #CODFORNECEDOR_fk, #CODFUNCIONARIO_fk}
...
-- ITENSCOMPRAPRODUTO = {@(#CODCOMPRA_fk, #CODPRODUTO_fk), QUANTIDADE, VALORC}
CREATE TABLE itenscompraproduto (
    codcompra_fk INTEGER NOT NULL REFERENCES compraproduto(codcompra),
    codproduto_fk INTEGER NOT NULL REFERENCES produto(codproduto),
    PRIMARY KEY (codcompra_fk, codproduto_fk),
    quantidade INTEGER NOT NULL,
    valorc numeric(10,2) NOT NULL check(valorc > 0)
);
...
-- PARCELACOMPRA = {@{CODPARCELACOMPRA  #CODCOMPRA_fk}, DATAVENCIMENTO, VALOR, #CODSITUACAO_fk}
CREATE TABLE parcelacompra (
    codparcelacompra INTEGER NOT NULL,
	codcompra_fk INTEGER NOT NULL REFERENCES compraproduto(codcompra),
	PRIMARY KEY(codparcelacompra, codcompra_fk),
    datavencimento DATE NOT NULL,
    valor numeric(10,2) NOT NULL check(valor > 0),
    codsituacao_fk INTEGER NOT NULL REFERENCES situacao(codsituacao)
);


-- VENDAPRODUTO = {@CODVENDA, DATAVENDA, #CODCLIENTE_fk, #CODFUNCIONARIO_fk}
CREATE TABLE vendaproduto (
    codvenda INTEGER IDENTITY PRIMARY KEY,
    datavenda DATE NOT NULL,
    codcliente_fk INTEGER NOT NULL REFERENCES cliente(codcliente),
    codfuncionario_fk INTEGER NOT NULL REFERENCES funcionarios(codfuncionario)
);


-- ITENSVENDAPRODUTO = {@(#CODVENDA_fk, #CODPRODUTO_fk), QUANTIDADE, VALOR}
CREATE TABLE itensvendaproduto(
    codvenda_fk INTEGER NOT NULL REFERENCES vendaproduto(codvenda),
    codproduto_fk INTEGER NOT NULL REFERENCES produto(codproduto),
...
-- PARCELAVENDA={@{CODPARCELA #CODVENDA_fk}, DATAVENCIMENTO, VALOR, #CODSITUACAO_fk}
CREATE TABLE parcelavenda(
    codparcela INTEGER NOT NULL,
	codvenda_fk INTEGER NOT NULL REFERENCES vendaproduto(codvenda),
	PRIMARY KEY(codparcela, codvenda_fk),
    datavencimento DATE NOT NULL,
    valor numeric(10,2) NOT NULL check(valor > 0),
    codsituacao_fk INTEGER NOT NULL REFERENCES situacao(codsituacao)
);
...
-- IMAGENS={@CODIMAGENS, IMAGEM, DESCRICAO, #CODPRODUTO_fk}
...
-- RUA = {@CODRUA, NOMERUA}
INSERT INTO rua(nomerua) VALUES('Rua 21');
...
-- CIDADE = {@CODCIDADE, NOMECIDADE, #CODUF_fk}
INSERT INTO cidade(nomecidade, coduf_fk) VALUES
('Afonso Cláudio', 8),
...
('Wanderlândia', 27),
('Xambioá', 27);
SELECT * FROM cidade WHERE nomecidade LIKE 'SANTA FÉ DO SUL';


-- LOJA = {@CODLOJA, NOMELOJA, CNPJ, NOMEFANTASIA, RAZAOSOCIAL}
insert into loja(nomeloja, cnpj, nomefantasia, razaosocial) 
values('HrSoluções', '77.923.657/0001-06', 'HrSoluções', 'HR Solucoes Markup');


-- FUNCAO = {@CODFUNCAO, NOMEFUNCAO}
INSERT INTO funcao(nomefuncao) VALUES('Admin');


-- FUNCIONARIOS = {@CODFUNCIONARIO, NOMEFUNCIONARIO, NUMEROCASA, #CODRUA_fk, #CODBAIRRO_fk, #CODCEP_fk, #CODCIDADE_fk, #CODFUNCAO_fk, SALARIO, #CODLOJA_fk}
insert into funcionarios(nomefuncionario, numerocasa, codrua_fk, codbairro_fk, codcep_fk, codcidade_fk, codfuncao_fk, salario, codloja_fk) 
values('Gabriel Ribeiro Maschio', 653, 1, 1, 1, 5229, 1, 1420, 1);
SELECT * FROM funcionarios;
SELECT nomefuncionario, numerocasa, nomerua, nomebairro, nomecidade, numerocep, salario, nomeloja, nomefuncao  
FROM funcionarios, rua, bairro, cep, cidade, loja, funcao
WHERE codrua_fk = codrua AND 
	  codbairro_fk = codbairro AND 
	  codcidade_fk = codcidade AND 
	  codcep_fk = codcep AND 
	  codloja_fk = codloja AND
	  codfuncao_fk = codfuncao;


-- LOGAR = {@CODLOGIN, USUARIO, SENHA, #CODFUNCIONARIO_fk}
INSERT INTO logar(usuario, senha, codfuncionario_fk) values('Admin', 123, 1);
INSERT INTO logar(usuario, senha, codfuncionario_fk) values('gabriel', 123, 1);

SELECT * FROM logar;


-- ACESSO = {@CODACESSO, NOMEACESSO}
INSERT INTO acesso(nomeacesso) VALUES('Admin');
INSERT INTO acesso(nomeacesso) VALUES('Fornecedor');
INSERT INTO acesso(nomeacesso) VALUES('Produto');
INSERT INTO acesso(nomeacesso) VALUES('Compra');
INSERT INTO acesso(nomeacesso) VALUES('Venda');
INSERT INTO acesso(nomeacesso) VALUES('Funcionario');
INSERT INTO acesso(nomeacesso) VALUES('Cliente');
INSERT INTO acesso(nomeacesso) VALUES('Endereco');
INSERT INTO acesso(nomeacesso) VALUES('Loja');
...
-- ITENSACESSOLOGIN = {@(#CODACESSO_fk, #CODLOGIN_fk)}
INSERT INTO itensacessologin(codacesso_fk, codlogin_fk) VALUES(1, 1);
INSERT INTO itensacessologin(codacesso_fk, codlogin_fk) VALUES(8, 2);
INSERT INTO itensacessologin(codacesso_fk, codlogin_fk) VALUES(12, 2);
INSERT INTO itensacessologin(codacesso_fk, codlogin_fk) VALUES(11, 2);
